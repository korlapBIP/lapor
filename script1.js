  ...
  if(table){
    ...
    if(thead){
      thead.innerHTML = isCommercialRows
        ? '<tr><th>NO</th><th>LD-Regu</th><th>NIP</th><th>NAMA PEKERJA</th><th>SHIFT 1</th><th>SHIFT 2</th><th>SHIFT 3</th><th>CEK IN</th><th>CEK OUT</th></tr>'
        : (isBahanBakuRows ? '<tr><th>NO</th><th>NIP</th><th>KEGIATAN</th><th>NAMA</th><th>SHIFT 1</th><th>SHIFT 2</th><th>SHIFT 3</th><th>CEK IN</th><th>CEK OUT</th></tr>' : '<tr><th>NO</th><th>NIP</th><th>NAMA</th><th>SHIFT 1</th><th>SHIFT 2</th><th>SHIFT 3</th><th>CEK IN</th><th>CEK OUT</th></tr>');
    }
    if(foot){
      foot.innerHTML = isCommercialRows
        ? '<tr><td class="summary-empty" colspan="4"></td><th>SHIFT 1</th><th>SHIFT 2</th><th>SHIFT 3</th><td class="summary-empty" colspan="2"></td></tr><tr><td class="left-label" colspan="4">JUMLAH PEKERJA</td><td id="sumS1">0</td><td id="sumS2">0</td><td id="sumS3">0</td><td class="summary-empty" colspan="2"></td></tr><tr><td class="summary-empty" colspan="4"></td><td class="total" colspan="3"><span id="sumTotal">0</span> Orang</td><td class="summary-empty" colspan="2"></td></tr>'
        : (isBahanBakuRows ? '<tr><td class="summary-empty" colspan="4"></td><th>SHIFT 1</th><th>SHIFT 2</th><th>SHIFT 3</th><td class="summary-empty" colspan="2"></td></tr><tr><td class="left-label" colspan="4">JUMLAH PEKERJA</td><td id="sumS1">0</td><td id="sumS2">0</td><td id="sumS3">0</td><td class="summary-empty" colspan="2"></td></tr><tr><td class="summary-empty" colspan="4"></td><td class="total" colspan="3"><span id="sumTotal">0</span> Orang</td><td class="summary-empty" colspan="2"></td></tr>' : '<tr><td class="summary-empty" colspan="3"></td><th>SHIFT 1</th><th>SHIFT 2</th><th>SHIFT 3</th><td class="summary-empty" colspan="2"></td></tr><tr><td class="left-label" colspan="3">JUMLAH PEKERJA</td><td id="sumS1">0</td><td id="sumS2">0</td><td id="sumS3">0</td><td class="summary-empty" colspan="2"></td></tr><tr><td class="summary-empty" colspan="3"></td><td class="total" colspan="3"><span id="sumTotal">0</span> Orang</td><td class="summary-empty" colspan="2"></td></tr>');
    }
  }
  body.innerHTML='';
  rows.forEach((w,i)=>{
    ...
    tr.innerHTML = isCommercialRows
      ? `<td>${i+1}</td><td>${safeText(w.ldRegu || commercialLdReguLabel(w.loadingDock,w.regu))}</td><td>${safeText(w.nip)}</td><td class="name">${safeText(w.name)}</td><td>${w.s1?'<span class="tick">✓</span>':''}</td><td>${w.s2?'<span class="tick">✓</span>':''}</td><td>${w.s3?'<span class="tick">✓</span>':''}</td><td>${renderCheckCell(w,'in')}</td><td>${renderCheckCell(w,'out')}</td>`
      : (isBahanBakuRows ? `<td>${i+1}</td><td>${safeText(w.nip)}</td><td>${safeText(bahanBakuKegiatanLabel(w))}</td><td class="name">${safeText(w.name)}</td><td>${w.s1?'<span class="tick">✓</span>':''}</td><td>${w.s2?'<span class="tick">✓</span>':''}</td><td>${w.s3?'<span class="tick">✓</span>':''}</td><td>${renderCheckCell(w,'in')}</td><td>${renderCheckCell(w,'out')}</td>` : `<td>${i+1}</td><td>${safeText(w.nip)}</td><td class="name">${safeText(w.name)}</td><td>${w.s1?'<span class="tick">✓</span>':''}</td><td>${w.s2?'<span class="tick">✓</span>':''}</td><td>${w.s3?'<span class="tick">✓</span>':''}</td><td>${renderCheckCell(w,'in')}</td><td>${renderCheckCell(w,'out')}</td>`);
    body.appendChild(tr);
  });
  if(!rows.length){ body.innerHTML=`<tr><td colspan="${(isCommercialRows||isBahanBakuRows)?9:8}" style="height:42px;color:#65758b">${safeText(emptyMsg || 'Belum ada pekerja yang dipilih.')}</td></tr>`; }
...
function adminApplyAutoCheckTimes(){
  ...
  rows.forEach(r=>{
    ...
    if(r.s1 && (r.s2 || r.s3)){ r.checkIn=randomBeforeWorkTime(s1In,10,15); r.checkOut=randomAfterWorkTime(r.s3?s3Out:s2Out,5,15); }
    else if(r.s2 && r.s3){ r.checkIn=randomBeforeWorkTime(s2In,10,15); r.checkOut=randomAfterWorkTime(s3Out,5,15); }
    else if(r.s1){ r.checkIn=randomBeforeWorkTime(s1In,10,15); r.checkOut=randomAfterWorkTime(s1Out,5,15); }
    else if(r.s2){ r.checkIn=randomBeforeWorkTime(s2In,10,15); r.checkOut=randomAfterWorkTime(s2Out,5,15); }
    else if(r.s3){ r.checkIn=randomBeforeWorkTime(s3In,10,15); r.checkOut=randomAfterWorkTime(s3Out,5,15); }
  });
  updateAdminReportRowsWithCheckTimes(rows);
  renderReport();
  alert('Cek In / Cek Out otomatis sudah diisi acak sesuai ketentuan Shift 1, Shift 2, dan Shift 3. Kegiatan Stapel (TF) dan Malleti (TF) tetap dikosongkan. Klik Simpan Cek In / Cek Out untuk menyimpan ke database.');
}
